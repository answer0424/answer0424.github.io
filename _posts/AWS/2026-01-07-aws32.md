...
| 항목 | 요약 |
|-|-|
| 목적 | AMI ID를 직접 지정하지 않고 Systems Manager Parameter Store를 통해 최신/지정 AMI 참조 |
| 파라미터 데이터 유형 | `aws:ec2:image` |
| 인슽너스 시작 구문 | `resolve:ssm:parameter-name[:version]` |
| IAM 권한 | `ssm:DescribeParameters`, `ssm:GetParameter` |
| 리전/이름 품질 | 리전별 파라미터 생성, 이름 대소문자 구분 |