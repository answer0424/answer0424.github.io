...
- Sysprep은 Windows AMI 생성 시 시스템 고유 정보를 제거하고 일반화 ➡️ 특수화 ➡️ OOBE를 통해 표준화된 AMI를 준비함
- EC2Config 서비스는 Sysprep 호출, 관리자 계정 처리, RDP 비활성/재활성, 응답 파일 적용 등을 자동화함
- Sysprep 실행 후 인스턴스가 Stopped 상태가 되면, 해당 상태에서 AMI를 생성하면 됨