...
#### 2. 원본 권한 리스트를 토대로 대상 AMI 권한 설정

```bash
aws ec2 modify-image-attribute \
  --image-id target-ami-id \
...
| 항목 | 설명 |
|-|-|
| AMI 권한 복사란 | 소스 AMI의 launch permissions을 대상 AMI에 동일하게 적용하는 작업 |
| 콘솔/CLI/SDK | 콘솔에서 선택하거나, CLI/SDK로 `modify-image-attribute` 호출 |
| 리전 간 복사 | 리전 간 AMI 복사 시 권한 리스트가 자동 복사됨 |
| 공유 대상 | 특정 계정, 전체 공개, Marketplace 조건 확인 |
| 주의 | 유료 AMI/Marketplace AMI 권한 제한 가능 |