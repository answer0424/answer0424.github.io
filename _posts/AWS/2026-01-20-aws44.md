...
- AMI 저장 시 S3 버킷 뒤에 파일 경로를 추가할 수 있음
- 복원 요청에서도 버킷 + 객체 키 경로를 함께 지정하면 해당 위치에서 AMI 객체를 가져옴
- 예:

```bash
버킷: amzn-demo-bucket/path1/path2  
저장 객체 키: path1/path2/ami-0abcdef1234567890.bin
```
...
| 항목 | 설명 |
|-|-|
| AMI 저장 흐름 | `CreateStoreImageTask` ➡️ S3에 단일 객체로 저장 |
| 진행 상태 확인 | `DescribeStoreImageTasks` ➡️ 저장/복원 작업 상태 조회 |
| AMI 복원 흐름 | `CreateRestoreImageTask` ➡️ S3 객체에서 새 AMI로 복원 |
| S3 객체 메타데이터 | AMI 정보 포함 |
| 한도 | 저장 태스크: 1,200GB, 복원 테스크: 600GB |