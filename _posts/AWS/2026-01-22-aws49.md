...
- AMI 사용 현황 파악
  - 어떤 리소스가 AMI를 참조하는지 파악
- AMI 교체/업그레이드 계획
  - 구 버전 AMI를 사용하는 리소스를 식별하여 업데이트 가능
- AMI 삭제/정리 전 영향 분석
  - AMI 사용 리소스를 확인하여 안전한 삭제 판단
...
| 항목 | 내용 |
|-|-|
| 기능 | 특정 AMI를 참조하는 모든 AWS 리소스를 식별 |
| 지원 리소스 | EC2 인스턴스, 시작 템플릿, SSM 파라미터, Image Builder 이미지/컨테이너 레시피 |
| 사용 방식 | 콘솔 또는 CLI |
| IAM 권한 | `DescribeImageReferences` 등 관련 권한 필요 |
| 관리형 정책 | `AmazonEC2ImageReferencesAccessPolicy` |