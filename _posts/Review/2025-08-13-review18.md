  ...
  return (
    <>
      ...
      <Panel title="Etymology">
        The name comes from <span lang="k-KZ">алма</span>, the Kazakh word for "apple" and is often translated as "full of apples". In fact, the region surrounding Almaty is thought to be the ancestral home of the apple, and the wild <i lang="la">Malus sieversii</i> is considered a likely candidate for the ancestor of the modern domestic apple.
      </Panel>
    </>
  );
}
...
> 
> 이 원칙은 또한 `단일 진리의 원천`을 갖는 것으로 알려져 있음
> 
> 이는 모든 state가 한 곳에 존재한다는 의미가 아니라 그 정보를 가지고 있는 특정 컴포넌트가 있다는 것을 말함