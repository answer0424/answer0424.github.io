...
```java
@GetMapping(path = "/pets/{petId}", headers = "My-Header=myValue")
public Pet getPet(@PathVariable String petId) { … }
```
...
- 여러 패턴이 동일한 요청과 매칭될 경우, 내부적으로 `PathPattern.SPECIFICITY_COMPARATOR`가 더 구체적인 매핑을 우선으로 선택
- `**`나 `{*var}` 같은 catch-all` 패턴은 점수가 낮아 우선순위가 뒤로 밀림

<br>

## 커스터 매핑 어노테이션 및 등록

- `@GetMapping`등은 `@RequestMapping(method = RequestMethod.GET)`의 메타 어노테이션
- 고급 시나리오에서는 `RequestMappingHandlerMapping`을 직접 조작해 런타임에 핸들러 메서드를 동적으로 등록할 수도 있음

<br>

## 유의사항 및 팁

- WebFlux에서는 suffix 매칭이 기본적으로 지원되지 않으므로, URL 뒤 파일 확장자 기반 매칭 대신 쿼리 파라미터 혹은 헤더 기반 방식을 사용하는 것이 안전
- 컨트롤러 메서드는 가능한 HTTP 메서드별 어노테이션을 사용해 명확하게 정의하면 가독성과 유지보수성이 증가
- 복잡한 매핑 조건이 많아지면 이해하기 어려워질 수 있으므로, 경로 설계와 조건 지정을 단순하게 유지하는 것이 좋음
- 매핑 패턴이 많거나 복잡한 경우에는 우선순위에 유의해야 하며, 동일한 요청에 대해 중복된 매핑이 되지 않도록 설계해야 함