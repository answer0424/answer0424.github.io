...
- GET 방식 뿐 아니라 POST 방식에서도 쿼리 파라미터로 전달할 수 있지만, body 기반 JSON 데이터를 처리하려면 `@RequestBody`를 사용하는 것이 적합
- 다수의 파라미터를 한꺼번에 객체로 바인딩하고 싶다면 `@ModelAttribute` 또는 DTO를 사용하는 것이 코드 관리 측면에서 유리
- `List<String>` 또는 `MultiValueMap<String, String>`과 같은 타입을 사용하면 같은 이름의 쿼리 파라미터가 여러 개일 때 (`?tags=a&tags=b` 등) 모두 수집할 수 있음
- WebFlux 환경에서는 폼 데이터나 multipart 요청에서 `@RequestParam`만으로 바인딩 되지 않는 경우가 있으므로, 해당 경우에는 `ServerWebExchange` 등을 사용해 직접 처리하는 방법을 고려해야 함