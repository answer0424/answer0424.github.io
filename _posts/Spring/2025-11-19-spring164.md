---
layout: post
title: Model
date: 2025-11-19 09:00:00 +09:00
categories: [Spring, Reactive Stack, WebFlux, Annotated Controllers]
tags: [spring]
image:
    path: /assets/img/spring/Spring.png
---

## 개요

- `@ModelAttribute` 어노테이션은 다음과 같은 용도로 사용됨
  1. `@RequestMapping` 메서드의 메서드 인자에 사용되어, 모델에서 객체를 가져오거나 새로 생성하고 요청 파라미터 바인딩을 수행할 수 있음
  2. 메서드 레벨에서 `@ModelAttribute`를 사용하면, 해당 컨트롤러 내의 모든 `@RequestMapping` 메서드가 실행되기 전에 모델을 초기화하는 역할을 하ㅏㅁ
  3. `@RequestMapping` 메서드 위에 붙여서 반환값을 모델 속성을 간주하도록 할 수도 있음

<hr>

## 사용방식

#### 실행 시점

- `ModelAttribute` 메서드는 같은 컨트롤러 클래스 내의 `@RequestMapping` 메서드보다 먼저 실행됨
- `@ControllerAdvice`에 정의된 `@ModelAttribute` 메서드는 여러 컨트롤러에 걸쳐 공통적으로 모델을 초기화할 때 사용할 수 있음

<br>

#### 메서드 시그니처의 유연성

- `@ModelAttribute` 메서드는 `@RequestMapping` 메서드에서 사용하는 대부분의 인자 유형을 지원하ㅏ지만, `@ModelAttribute` 자신, 요청 본문 관련 인자 등은 사용할 수 없음

```java
@ModelAttribute
public void populateModel(@RequestParam String number, Model model) {
    model.addAttribute(accountRepository.findAccount(number));
}
```

또는

```java
@ModelAttribute
public Account getAccount(@RequestParam String number) {
    return accountRepository.findAccount(number);
}
```

- 위 두 방식 모두 가능하며, 후자의 경우 반환값 타ㅏ입이나 메서드 이름을 기반으로 기본 이름이 지정됨

<br>

#### 모델 속성 명명 및 반환값 처리

- `@ModelAttribute` 메서드에서 명시적으로 이름을 지정하지 않으면, 반환값 타입이나 메서드 이름을 기반으로 기본 이름이 지정됨
- 이름을 직접 지정하려면, 메서드 수준 `@ModelAttribute("명칭")`을 사용하거나 `Model.addAttribute("명칭", 객체)` 방식으로 속성 추가가 가능함

```java
@GetMapping("/account/{id}")
@ModelAttribute("myAccount")
public Account handle() {
    // ...
    return account;
}
```

<br>

#### `@RequestMapping` 메서드와 함께 사용하는 경우

- `@RequestMapping` 메서드에 `@ModelAttribute`가 붙이면, 그 메서드의 반환값이 모델 속성으로 간주됨
- 다만, 반환값이 `String`이고 그것이 뷰 이름으로 해석될 여지가 있는 경우엔 주의해야 함

<hr>

## 언제/왜 사용하나

- 요청 처리 전에 공통 데이터를 모델에 미리 세팅하고 싶을 때 유용
- 컨트롤러 내 여러 `@RequestMapping` 메서드에서 공통으로 사용하는 객체 또는 속성을 초기화하거나 준비할 때 코드 중복을 줄일 수 있음
- 모델 속성의 생성과 바인딩을 자동화하고, 요청 파라미터를 객체에 바인딩한 뒤 컨트롤러 메서드에서 바로 사용하고자 할때도 효과적임

<hr>

## 주의사항 및 팁

- 모델에 추가된 속성은 뷰에서 사용되거나 리다이렉트/포워드 시 재사용되므로, 너무 많은 데이터가 사전 추가되면 불필요한 메모리 사용 또는 성능 저하의 원인이 될 수 있음
- `ModelAttribute` 메서드가 여러 개 있을 경우 실행 순서는 명시되어 있지 않으므로, 메서드 간 의존관계가ㅏ 있다면 주의를 기울여야 함
- 반환 값으로 객체를 리턴하는 `@ModelAttribute` 메서드에서는 반환된 객체가 모델에 저장되므로, 메서드 내부에서 `Model.addAttribute()`를 사용 사용하는 방식과 동일한 효과가 있지만 코드 스타일 차이가 있음
- `@ModelAttribute` 메서드는 요청 바디를 처리하는 `@RequestBody` 방식이나 REST API의 경우에는 상대적으로 사용 빈도가 낮을 수 있음
- 모델 속성 네이밍 및 충돌에 유의해야 함 