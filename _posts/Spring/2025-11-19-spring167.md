...
- 예외 발생 시, `DispatcherServlet`이 등록된 `HandlerExceptionResolver` 체인을 통해 예외를 처리함
  - `ExceptionHandlerExceptionResolver`: `@ExceptionHandler` 적용 메서드를 호출
  - `ResponseStatusExceptionResolver`: `@ResponseStatus` 가 붙은 예외 처리
  - `DefaultHandlerExceptionResolver`: 스프링 내부 표준 예외 처리
- 따라서 `@ExceptionHandler` 메서드가 적절히 지정되어 있으면 해당 메서드가 예외 처리기로 활용됨