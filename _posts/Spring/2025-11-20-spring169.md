...
- 라우팅 규칙이 많고 복잡하여 라우터 구성이 분산된 경우 한 곳에서 라우터를 정리하고 싶을 때 유용함
- 어노테이션 기반 컨트롤러가 아닌 작은 서비스/마이크로서비스 형태로 라우팅을 빠르게 구성하고 싶을 때 
- 람다, 메서드참조 스타일을 선호하거나 함수형 프로그래밍 패러다임을 코드 스탕일로 채택하고 있는 경우