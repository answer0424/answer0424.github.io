...
- 컨트롤러가 반환하는 `Resource` 타입이 `InputSteamResource`일 경우, 범위 요청 지원이 동작하지 않을 수 있으므로 주의해야 함
- 개발자는 `Range` 헤더를 수동으로 처리하기보다는 Spring MVC가 제공하는 이 자동화된 기능을 활용하는 것이 유지보수 및 표준 준수 측면에서 유리함
- 브라우저나 미디어 플레이어가 `Accept-Ranges: bytes` 헤더나 `Content-Range` 헤더를 기대할 수 있으므로, 해당 처리가 올바르게 설정되어 있는지 테스트해 보는 것이 좋음
- 만약 커스텀 방식으로 리소스를 제공하고 있다면 Spring의 기본 처리 외에 별도 설정이 필요할 수 있음