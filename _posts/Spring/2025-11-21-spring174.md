...
- 캐싱을 잘못 설정하면 오래된 데이터를 제공할 위험이 있음. `Etag`나 `Last-Modified` 등의 조건부 헤더를 적극 활용
- `Cache-Control: no-store` 등으로 아예 캐시를 금지하는 경우도 적절히 고려해야 함
- 정적 리소스뿐 아니라 API 응답에도 캐싱이 가능하지만, API 응답은 데이터 변경 가능성을 고려해 캐시 기간이나 ETag 전략을 신중히 설계해야 함
- 필터나 인터셉터에서 캐시 헤더를 설정할 경우, URL 패턴별로 캐시 정책을 적용하는 것을 고려해 볼 수 있음 