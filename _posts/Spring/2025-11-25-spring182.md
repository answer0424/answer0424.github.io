...
- `.block()`을 남용하면 비동기 리액티브의 이점을 잃게 됨 ➡️ 특히 많은 요청을 순차적으로 block하는 것이 적합하지 않을 수 있음
- WebFlux 어플리케이션안에서는 가능한 블로킹 사용을 피하고, reactive 타입으로 끝까지 처리하는 것이 권장됨
- 동기 호출이 필요한 상황이라면, RestTemplate 또는 Spring에서 새로 나온 RestClient 같은 동기 클라이언트 사용도 고려해 볼 수 있음 