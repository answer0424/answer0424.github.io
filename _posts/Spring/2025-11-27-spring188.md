...
- embedded DB는 실제 운영과 다를 수 있음 ➡️ DB 벤더 특성 차이에 따른 미스매치 주의
- 너무 복자한 DB 구조나 SQL 고급 기능은 embedded DB에서 동작이 다를 수 있음 
- DAO 단위 수준에서 테스트가 끝나므로, 어플리케이션 전체 흐름까지 검증하려면 별도의 통합 테스트가 필요 

<hr>

## 실무 팁

- DAO/Repository를 구현할 때는 SQL 스크립트 + 테스트용 데이터 삽입 스크립트를 준비해두고, embedded DB 테스트에서 이를 실행해 기본 테이블 구조 및 초기 데이터를 세팅해두면 좋음
- 테스트 중 트랜잭션 + 자동 롤백 기능을 활용하면, 각 테스트가 독립적으로 데이터베이스를 초기 상태로 유지할 수 있어 테스트 간 간섭을 피할 수 있음
- DAO 로직 단위 테스트뿐만 아니라, 간단한 통합 흐름 테스트에도 화용하면 유용
- 만약 운영 DB와 spec 차이가 크다면 embedded DB 테스트 + 실제 DB를 대상한 별도의 통합 테스트를 조합하는 전략을 쓰는 것이 안전