...
- `ApplicationEvents`는 테스트 메서드 단위로 scope가 한정되어 있음
- 테스트 메서드가 끝나면 그 기록도 사라지며, 다른 테스트에서는 다시 기록이 새로 시작됨
- 만약 `@TestExecutionListeners`를 커스터마이즈하면서 기본 리스너 집합을 덮어 쓸 경우
  - `ApplicationEventsTestExecutionListener`를 반드시 포함하거나 등록해야 함
- 이벤트를 비동기적으로 publish하는 경우, 이벤트 로그가 테스트 메서드가 종료되기 전에 기록되지 않을 수 있음