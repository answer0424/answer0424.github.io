...
- `EventPublishingTestExecutionListener`는 기본 등록되어 있지만, `ApplicationContext`가 이미 로드된 경우에만 이벤트를 발행함
  - 즉, 처음 해당 컨텍스트가 로드되는 테스트 클래스에서는 `BeforeTestClassEvent`가 발행되지 않을 수 있음
- 만약 `@DirtiesContext` 등에 의해 컨텍스트가 캐시에서 제거될 경우 발행되지 않을 수 있음
- 이벤트 소비에서 예외가 발생하면 기본적으로 그 예외가 테스트 실패로 이어짐
- `@EventListener` 기반 리스너는 일반 Spring 빈이므로, 의존성 주입, 다른 빈 참조 등 Spring의 모든 기능을 활용할 수 있음

<hr>

## 정리

- 모든 테스트 클래스에 대해 공통 전처리/후처리 로직을 적용하고 싶을 때
- 특정 테스트 메서드의 실행 흐름을 감시하거나, 테스트 내부 동작 흐름을 외부에서 모니터링하고자 할 때
- DI가 필요한 처리 로직을 테스트 후처리 코드에서도 사용하고 싶을 때 - 일반 `TestExecutionListener`로는 어렵고 bean 기반 리스너가 유리
- 테스트 프레임워크에 독립적인 방식으로 후처리로직을 통합하고자할 때