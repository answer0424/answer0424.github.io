...
- 컨텍스트 재사용의 위험: 캐시된 컨텍스트를 재사용하기 때문에, 테스트 중에 빈 설정을 변경하거나 Mock/Spy를 동적으로 추가/제거하면 예상치 못한 공유 상태 문제가 생길 수 있음
  - 이런 경우 `@DirtiesContext` 어노테이션을 사용해 컨텍스트를 더럽힌 것으로 표시하고 이후 테스트에서 새 컨텍스트로 재로드하도록 강제하는 것이 안전
- XML 설정 ➡️ 자바 설정 전환 고려: 레거시 프로젝트에 XML 설정이 많지만 점점 Java Config 혹은 Spring Boot 스타일 설정으로 전환하는 것이 유지보수와 가독성 면에서 유리할 수 있음
- 테스트 속도 최적화 전략:
  - 공통 XML 설정을 여러 테스트에서 공유해서 캐싱 효과를 활용
  - 불필요한 빈이나 설정을 줄여 가벼운 Context 구성
  - 테스트 목적에 따라 config 분리 - 예: 웹 계층 테스트는 servlet-context.xml, 서비스/DAO 테스트는 root-context.xml 등으로 분리