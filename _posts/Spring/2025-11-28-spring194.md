...
- Groovy 의존성 필요: Groovy가 클래스패스에 포함되어 있어야만 Groovy-based context loader가 활성화됨. 없으면 작동하지 않음
- Spring 기본 로더 제한: 표준 Spring 배포판이 제공하는 `SmartContextLoader` 구현체들은 보통 XML, 자바 설정, 또는 Groovy 중 하나만을 엔트리 포인트로 지원. 만약 여러 설정 방식을 혼합하려면 주의가 필요함. 다행히 `GenericGroovyXmlContextLoader` 는 XML + Groovy 혼용을 지원함
- IDE / 빌드 설정 주의: 테스트에 Groovy 파일을 쓸 경우, 빌드 스크립트(Gradle/Maven)와 IDE 설정이 Groovy를 인식하도록 정확히 구성되어야 함
- 타입 안전성 부족: 자바 설정(@Configuration + @Bean)처럼 컴파일 시 타입 체크가 엄격하지 않으므로, 런타임 오류 가능성에 유의해야 함