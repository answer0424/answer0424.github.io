...
- 타입 안정하고 IDE 지원이 풍부 ➡️ 리팩토링이나 코드 분석이 쉬움
- 테스트 전용 설정을 Java 코드로 정의하여, XML 대신 더 직관적으로 유지보수 가능 
- 실제 어플리케이션 설정 클래스를 재사용하거나, 테스트 전용 config를 별도 분리하여 관리할 수 있음 
- TestContext의 기능과 자연스럽게 연동됨

<hr>

## 주의사항

- 설정 클래스를 잘못 정의하면 bean 충돌, 의존성 주입 실패, 설정 누락 등이 생김 - 특히 빈 스코프, 컴포넌트 스캔 범위, 프로파일 설정 등 주의 필요
- 혼합 설정을 사용할 경우, 빈 정의/우선순위/충돌 관리에 신경 써야 함
- 테스트 중 빈 상태를 변경하면 캐시된 Context를 공유하고 있기 때문에 테스트 간 간섭이 발생할 수 있음 