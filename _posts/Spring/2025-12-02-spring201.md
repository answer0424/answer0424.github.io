...
- 테스트 환경마다 조금씩 다른 설정이 자주 필요할 때
- 동일 설정 + 약간의 variation이 있는 테스트 클래스들이 많고, 중복을 줄이고 싶을 때 
- configuration 설정을 코드로 관리하고, 변경 이력을 명확히 관리하고 싶을 떄
- 테스트 설정이 복잡해지고, 단순 `@ContextConfiguration(classes=...)` 또는 XML/Groovy config 만으로 표현하기 어려울 때