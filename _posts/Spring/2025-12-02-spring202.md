...
#### 3. 설정 클래스 없이 오직 initializer만 사용 가능

> 설정 클래스나 XML 없이도, `ApplicationContextInitializer` 만으로 빈 정의/등록을 모두 처리할 수 있음
...
- 이 경우 `EntireAppInitializer` 내부에서 필요한 빈 동의를 모두 등록하는 책임을 가짐

<hr>

## 주의사항 & 유의할 점

- initializer에서 등록하는 빈이나 설정은, 사용하는 `ApplicationContext` 타입과 호환되어야 함
- 여러 Initializer를 지정할 때, 실행 순서를 제어하고 싶다면 `Ordered` 인터페이스를 구현하거나 `@Order`, `@Priority` 어노테이션을 사용해야 함
- 테스트 컨텍스트 캐싱을 사용할 때, Initializer가 설정한 환경도 캐시 키의 일부가 됨 