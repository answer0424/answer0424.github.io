...
## 예제: Web MVC 통합 테스트 with Context Hierarchy

```java
// root configuration (common services, datasource, repository, etc.)
@Configuration
public class RootConfig {
    // @Bean definitions: service, repository, dataSource, transactionManager etc.
}

// servlet/web configuration (controllers, viewResolvers, handlerMappings, etc.)
@Configuration
@EnableWebMvc
...
public class MyControllerIntegrationTest {
    ...
    @Test
    void testSomeController() throws Exception {
...
| 상황 / 목적 | 추천 방식 |
|-|-|
| 단순 서비스 + repository + 단일 설정 ➡️ 서비스/비즈니스 로직 테스트  | 단일 `ApplicationContext` |
| Web MVC 또는 모듈화된 구조 | Context Hierarchies - 역할 분리 + 재사용 + 명확한 구조 |
| 여러 테스트 클래스가 공통 설정 + 각자 다른 하위 설정을 가질 때 | 부모 테스트 + 자식 테스트 + 계층 설정 |
| 테스트 간 설정 단순 + 속도 우선 | 단일 컨텍스트 + 캐싱 | 