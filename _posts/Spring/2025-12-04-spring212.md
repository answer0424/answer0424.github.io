...
public class MyRepositoryTest {
    ...
    void testInsertAndFind() {
        ...
        // when
        Optional<MyEntity> found = repo.findById(e.getId());
...
- 위처럼 하면, `repo.save()`로 DB에 insert해도, 테스트가 끝나면 자동으로 롤백 ➡️ DB 상태는 유지
- 만약 특정 테스트만 커밋하고 싶다면

```java
@Test
@Transactional
@Commit  // 또는 TestTransaction.flagForCommit()
void testCommitData() {
    // DB 변경이 실제로 커밋됨
}
```
...
- TestContext Framework의 트랜잭션 관리 기능은, 테스트를 더욱 안정적이고, 격리된 환경에서 실행하게 하게 해줌
- `@Transactional` + `PlatformTransactionManager` 설정만으로 트랜잭션 + 롤백이 기본 동작 ➡️ DB 상태 격리 쉬움
- 필요 시 `TestTransaction`으로 커밋/롤백 제어 또는 테스트 내 트랜잭션을 세밀 조정 가능
- 다만 일부 트랜잭션 속성 제한, 스레드 / timeout 설정 주의, 생명주기 제한 등 고려해야 할 점도 있음