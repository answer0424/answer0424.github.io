...
## 어떤 경우에 어떤 방식을 쓰면 좋을까

| 상황 / 용도 | 권장 방식 |
|-|-|
| 테스트마다 동일한 스키마 / 기본 데이터가 필요 → 반복 작성 줄이고 싶을 때 | `@Sql` 선언 방식 |
| 테스트마다 다른 데이터나 여러 단계를 수동으로 제어해야 할 때 (조건에 따라 스크립트 실행/롤백 등) | `ResourceDatabasePopulator` (프로그래밍 방식) |
| 테스트 중간에 DB 상태 변경 + 조회 + 검증 + 다시 다른 쿼리 실행이 필요한 경우 | 프로그래밍 방식 (직접 스크립트 실행) |
| 단순 테이블 생성 + 기본 데이터 삽입 + CRUD 테스트 | `@Sql` + 트랜잭션 + 롤백 조합 |
...
- `@Sql`은 기본적으로 테스트 메서드 실행 전후만 지원
- 만약 테스트가 `@Transactional` + 자동 롤백 모드라면, `@Sql`로 삽입한 데이터도 롤백됨 ➡️ 테스트 메서드 내에 데이터가 유지되지 않을 수 있음
- 스크립트 파일의 경로 지정 시 클래스패스 상대/절대 구분을 잘 해야 함
- 여러 테스트가 병렬로 실행되거나, 캐시된 `ApplicationContext`를 공유하는 경우 - 스크립트 실행으로 인한 DB 상태 변경이 의도치 않게 서로 영향을 줄 수 있음