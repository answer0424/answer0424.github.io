...
- REST API / 웹 컨트롤러 로직 + JSON/XML 응답 + validation + 예외 핸들링 등 웹 레이어 + 직렬화 역직렬화 + Spring MVC 흐름을 검증하고 싶을 때
- 실제 서버를 띄우기엔 무겁고 느린 통합 테스트보다, 빠르고 독립적인 단위 또는 슬라이스 테스트가 필요할 때
- CI 환경 또는 로컬 개발 환경에서 반복 테스트를 빠르게 실행하고 피드백을 받고 싶을 때
- 보안 필터, 인터셉터, message converter, 데이터 바인딩, validation 등 Spring MVC 핵심 기능이 제대로 설정되었는지 빠르게 검증하고 싶을 때

<hr>

## 정리

> MockMvc는 서버 없이도 Spring MVC 전체 요청/응답 흐름을 거의 그대로 흉내내는 테스트 도구
>
> 덕분에 컨트롤러 + 핸들러 매핑, 바인딩, validation, 예외 핸들러 등 실제 웹 요청과 유사한 수준의 테스트를 가볍고 빠르게 작성할 수 있음
>
> 다만 전체 실행 환경을 완전하게 대체하지는 않기 때문에 view 렌더링, 서버-specific 설정, 실제 네트워크 관련 동작 등을 검증하려면 별도의 통합/엔드투엔드 테스트가 필요