...
- 일반 GET/POST/PUT/DELETE 등 HTTP 메서드 요청
- 파일 업로드 - 실제 multipart 파싱이 아니라 mock 형태로 테스트 가능
- 쿼리 파라미터 전달
- 폼 파라미터 또는 일반 Servlet request 파라미터 전달
- contextPath, servletPath 같이 서블릿 경로 설정 포함 ➡️ 실제 서블릿 매핑이 된 환경과 유사하게 테스트

<hr>

## 장점

- 서버를 띄우지 않고도 Spring MVC 컨트롤러 + 관련 설정 전체를 mock 환경에서 테스트할 수 있음
- 다양한 HTTP 메서드, multipart, 파라미터 방식 등에 유연하게 대응
- 기본 속성 설정 덕분에 반복되는 설정을 줄이고, 테스트 코드 가독성 / 재사용성 향상
