...
- 실제 서블릿 컨테이너 없이 동작하는 `MockMvc`는, 비동기 요청 처리가 완료되더라도 자동으로 dispatch를 수행하지 않음
- 만약 이 수동 dispatch 과정을 생략하면 실제 비동기 결과가 응답되지 않거나 테스트가 실패하거나, 예기치 않은 동작이 일어날 수 있음
- 따라서 asyncStarted ➡️ asyncResult ➡️ asyncDispatch ➡️ 최종 검증의 순서를 꼭 지켜야 함

<hr>

## 주의 사항

- 이 방식 비동기 처리형 컨트롤러 메서드 - 예: 반환 타입이 `DeferredResult`, `Callable` 또는 Reactor `Mono`/`Flux`등인 경우에 유용함
- 만약 streaming 응답 등을 테스트하고 싶다면, 별도 권장되는 방식이 있음
- `MockMvc` + async 테스트는 fully mocked servlet environment에 기반하므로, 실제 서버를 띄우지 않고도 비동기 요청 흐름을 검증할 수 있다는 점이 장점