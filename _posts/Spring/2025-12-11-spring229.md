...
```java
// 기존 MockMvc + AssertJ 통합 방식 1: perform + assertThat
assertThat(mockMvc.perform(get("/hotels/{id}", 42)))
    .hasStatusOk();
...
// 방식 3: custom result handler 재사용
assertThat(mockMvc.perform(post("/save").param("name", "John")))
    .apply(myCustomResultHandler());
```
...
- 이미 `MockMvc` 중심으로 테스트 코드가 많고 완저히 새로 바꾸기엔 부담 있을 때
- 점진적으로 `AssertJ` + `fluent API` 스타일로 마이그레이션 하고 싶을 때
- custom matcher/handler를 많이 써서 기존 코드를 유지하면서도 새로운 asertion 기법을 도입하고 싶을 때
- 테스트 스타일 통일 + 가독성/유지보수성 향상을 위해 기존 + 새 방식의 하이브리드 전략을 택하고 싶을 때